...
export default function PersonalProfilePage() {
  ...
  // Active sub-tab
  const [activeTab, setActiveTab] = useState<'overview' | 'security' | 'sessions' | 'preferences' | 'subscription' | 'activity' | 'privacy' | 'timeline'>('overview');
  ...
  const handleUpdateAvatar = async (e: React.FormEvent) => {
    ...
    setActionLoading(true);
    try {
      const res = await apiClient.post('/profile/avatar', { avatarUrl: avatarFormUrl });
      setProfile(res.data.data);
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Password updated successfully. You have been logged out of all devices. Redirecting to login...", "success");
  ...
  const handleRevokeAllSessions = () => {
    setConfirmAction({
      ...
      message: "Terminate all device connections? This will log you out of your current session as well.",
      onConfirm: async () => {
        try {
  ...
  const handleDeactivateAccount = () => {
    setConfirmAction({
      ...
      message: "Deactivating your account will block login access immediately. You can contact support to reactivate. Proceed?",
      onConfirm: async () => {
        try {
  ...
  const handleDeleteRequest = () => {
    setConfirmAction({
      ...
      message: "This will submit a ticket to delete your personal data permanently. This action is irreversible. Proceed?",
      onConfirm: async () => {
        try {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-5">
            <div className="relative group">
              ...
              <div className="absolute inset-0 bg-black/40 rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center transition duration-150 cursor-pointer" onClick={() => setIsEditingAvatar(true)}>
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                {profile?.roles.map((r, idx) => (
                  <span key={idx} className="bg-primary/5 text-primary px-2.5 py-0.5 rounded-full font-bold border border-primary/10 capitalize">
                    {r}
                  </span>
                ))}
        ...
        {isEditingAvatar && (
          <div className="mt-4 p-4 border border-border/80 rounded-xl bg-background/50 max-w-md">
            <form onSubmit={handleUpdateAvatar} className="space-y-3">
              <label className="block text-xs font-bold text-text uppercase">Avatar Image URL</label>
              <div className="flex gap-2">
                <input
                  type="url"
                  required
        ...
        {isEditingProfile && (
          <form onSubmit={handleUpdateProfile} className="mt-6 border-t border-border pt-6 space-y-4 max-w-xl">
            <h3 className="text-sm font-bold text-text uppercase tracking-wider">Update Personal Details</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-bold text-text-light uppercase mb-1">Display Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-text-light uppercase mb-1">Country</label>
                <input
                  type="text"
            ...
            {activeTab === "overview" && profile && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Overview of your registered personal and corporate data.</p>
                </div>
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-4">
                    {[
                      ...
                      { label: "Account Type", value: profile.accountType.toUpperCase() },
                    ].map((item, idx) => (
                      <div key={idx} className="border-b border-border/60 pb-3">
                        <span className="text-xs font-bold text-text-light block uppercase tracking-wider">{item.label}</span>
                        <span className="text-sm font-semibold text-text mt-1 block">{item.value}</span>
                      </div>
                    ))}
                  </div>
                  <div className="space-y-4">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4 max-w-xl">
                  <div>
                    ...
                    <p className="text-xs text-text-light font-medium">Update your account access credentials regularly.</p>
                  </div>
                  <form onSubmit={handleChangePassword} className="space-y-4">
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-bold text-text-light uppercase mb-1">Current Password</label>
                        <input
                          type="password"
                          required
                      ...
                      <div>
                        <label className="block text-xs font-bold text-text-light uppercase mb-1">New Password</label>
                        <input
                          type="password"
                          required
                      ...
                      <div>
                        <label className="block text-xs font-bold text-text-light uppercase mb-1">Confirm New Password</label>
                        <input
                          type="password"
                          required
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-border pb-4">
                  <div>
                    ...
                    <p className="text-xs text-text-light">Current login access keys active for your account.</p>
                  </div>
                  <button
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {sessions.map(s => (
                    <div key={s.id} className="p-4 border border-border rounded-xl bg-background/50 flex flex-col justify-between space-y-3">
                      <div>
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-sm text-text leading-snug">{s.userAgent || "Web Browser"}</span>
                          {s.isCurrent && (
                        ...
                        <span className="text-xs text-text-light block mt-1">IP Address: {s.ipAddress}</span>
                        <span className="text-xs text-text-light block">Authorized: {new Date(s.createdAt).toLocaleString()}</span>
                      </div>
                      {!s.isCurrent && (
                ...
                <div className="space-y-4 pt-6 border-t border-border">
                  <div>
                    <h3 className="text-sm font-bold text-text uppercase tracking-wider">Trusted Device History</h3>
                    <p className="text-xs text-text-light">History of device fingerprints used to sign in.</p>
                  </div>

                  {devices.length === 0 ? (
                    <p className="text-xs text-text-light italic">No registered devices on record.</p>
                  ) : (
                    <div className="space-y-2">
                      {devices.map(d => (
                        <div key={d.id} className="p-3 border border-border/60 rounded-xl flex items-center justify-between text-xs bg-background/20">
                          <div className="flex items-center gap-3">
                            <Smartphone size={18} className="text-text-light" />
                            <div>
                              <span className="font-bold text-text block">{d.deviceName || "Personal Device"}</span>
                              <span className="text-text-light block">OS: {d.osName} • Browser: {d.browserName}</span>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-text-light block">IP: {d.lastIpAddress}</span>
                            <span className="text-[10px] text-text-light block">Last Active: {new Date(d.lastActiveAt).toLocaleDateString()}</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {activeTab === "preferences" && (
              <div className="space-y-6 max-w-xl">
                <div>
                  ...
                  <p className="text-xs text-text-light">Configure notifications for different channel updates.</p>
                </div>

                <div className="space-y-3">
                  {[
                    { key: "email", label: "Email Alerts", description: "Receive transaction alerts and account confirmations in your inbox." },
                    { key: "push", label: "Push Notifications", description: "Get real-time browser action alerts immediately." },
                    { key: "sms", label: "SMS Messages", description: "Deliver urgent alerts directly to your phone." },
                    { key: "marketing", label: "Marketing Emails", description: "Weekly campaigns, offers, and platform enhancements." },
                    { key: "security", label: "Security & Login Alerts", description: "Crucial sign-in logs, password updates, and login alerts." },
                    { key: "tender", label: "RFP / Tender Updates", description: "New match alerts, invitations, and bidding activity updates." },
                    { key: "newsletter", label: "Monthly Newsletter", description: "Monthly insights and bid statistics analysis digests." },
                  ].map((pref) => (
                    <div key={pref.key} className="flex items-start justify-between p-3 border border-border/60 rounded-xl bg-background/20">
                      <div className="space-y-0.5">
                        <span className="font-bold text-sm text-text block">{pref.label}</span>
                        <span className="text-xs text-text-light block leading-normal">{pref.description}</span>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer mt-1">
                        <input
                          type="checkbox"
                          checked={preferences[pref.key as keyof NotificationPreferences]}
                          onChange={(e) => handlePreferenceChange(pref.key as keyof NotificationPreferences, e.target.checked)}
            ...
            {activeTab === "subscription" && subscription && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Current tier subscription and billing range.</p>
                </div>
                ...
                <div className="p-5 border border-border rounded-2xl bg-background/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-light">Current Plan</span>
                  ...
                  <div className="text-xs font-semibold text-text-light space-y-1 sm:text-right">
                    <div>Status: <span className="text-emerald-500 font-bold uppercase">{subscription.status}</span></div>
            ...
            {activeTab === "activity" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Access audit logs to review logins and profile interactions.</p>
                </div>

                <div className="grid gap-6 lg:grid-cols-2">
                  <div className="space-y-3">
                    <h4 className="text-sm font-bold text-text border-b border-border pb-2">Activity Events</h4>
                    {activities.length === 0 ? (
                      <p className="text-xs text-text-light italic">No active events logged.</p>
                    ) : (
                      activities.map(a => (
                        <div key={a.id} className="p-3 border border-border/50 rounded-xl bg-background/30 text-xs">
                          <div className="flex justify-between items-center font-bold">
                            <span className="text-text">{a.action}</span>
                            <span className="text-text-light font-normal text-[10px]">{new Date(a.createdAt).toLocaleString()}</span>
                          </div>
                          {a.details && (
                  ...
                  <div className="space-y-3">
                    <h4 className="text-sm font-bold text-text border-b border-border pb-2">Security Audit History</h4>
                    {securityLogs.length === 0 ? (
                      <p className="text-xs text-text-light italic">No security history logs.</p>
                    ) : (
                      securityLogs.map(s => (
                        <div key={s.id} className="p-3 border border-border/50 rounded-xl bg-background/30 text-xs space-y-1">
                          <div className="flex justify-between items-center font-bold">
                            <span className="text-text">{s.event}</span>
                            <span className="text-text-light font-normal text-[10px]">{new Date(s.createdAt).toLocaleString()}</span>
                          </div>
                          <span className="text-[10px] text-text-light block">IP: {s.ipAddress || "Internal"} • Agent: {s.userAgent || "Unknown"}</span>
                        </div>
                      ))
                    )}
                  </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4 max-w-xl">
                  <div>
                    ...
                    <p className="text-xs text-text-light font-medium">To modify locked attributes (e.g. Email, Company Registration, Tax ID), submit a change request ticket.</p>
                  </div>
                  <form onSubmit={handleRequestChange} className="space-y-3 p-4 border border-border rounded-xl bg-background/30">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
                        <label className="block text-xs font-bold text-text-light uppercase mb-1">Target Field</label>
                        <select
                      ...
                      <div>
                        <label className="block text-xs font-bold text-text-light uppercase mb-1">Requested Value</label>
                        <input
                          type="text"
                          required
                          value={changeRequestValue}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-text-light uppercase mb-1">Reason / Supporting Proof Details</label>
                      <textarea
                        required
                ...
                <div className="border-t border-border pt-6 max-w-xl space-y-4">
                  <div>
                    <h3 className="text-sm font-bold text-text uppercase tracking-wider">Connected Accounts & OAuth</h3>
                    <p className="text-xs text-text-light">Manage sign-in integrations with external OAuth authentication providers.</p>
                  </div>
                  <div className="space-y-2">
                    {[
                      { provider: "Google", desc: "Sign in with your Google Workspace profile." },
                      { provider: "Microsoft", desc: "Connect Azure Active Directory profile." },
                      { provider: "GitHub", desc: "Enable developer account sign-in mapping." },
                    ].map((app) => (
                      <div key={app.provider} className="flex items-center justify-between p-3 border border-border/60 rounded-xl bg-background/25">
                        <div className="flex items-center gap-3">
                          <Link2 size={16} className="text-text-light" />
                          <div>
                            <span className="font-bold text-xs text-text block">{app.provider}</span>
                            <span className="text-[10px] text-text-light block mt-0.5">{app.desc}</span>
                          </div>
                        </div>
                ...
                <div className="border-t border-border pt-6 max-w-xl space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-text-light">Deactivate or request permanent erasure of your personal account.</p>
                  </div>
            ...
            {activeTab === "timeline" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Personal account progress checkpoints and security milestones.</p>
                </div>
