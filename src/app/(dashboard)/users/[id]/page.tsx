...
import { 
  ArrowLeft, User, Shield, Key, HardDrive, CreditCard, 
  BookOpen, Activity, Clock, ShieldAlert, Ban, UserCheck, 
  RotateCw, PlusCircle, Trash2, LogOut, CheckCircle, Smartphone
} from "lucide-react";
...
export default function UserDetailsPage() {
  ...
  // Active sub-tab
  const [activeTab, setActiveTab] = useState<'overview' | 'security' | 'sessions' | 'subscription' | 'notes' | 'activity' | 'timeline'>('overview');
  ...
  const fetchSubResource = async () => {
    ...
    try {
      ...
      if (activeTab === "overview") {
        const res = await apiClient.get(`/admin/users/${id}/overview`);
        setOverview(res.data.data);
      } else if (activeTab === "security") {
        const [secRes, rolesRes] = await Promise.all([
          apiClient.get(`/admin/users/${id}/security`),
          apiClient.get(`/admin/users/${id}/roles`),
        ]);
        setSecurity(secRes.data.data);
        setRoles(rolesRes.data.data?.assigned || []);
      } else if (activeTab === "sessions") {
        const [sessRes, devRes] = await Promise.all([
          apiClient.get(`/admin/users/${id}/sessions`),
          apiClient.get(`/admin/users/${id}/devices`),
        ]);
        setSessions(sessRes.data.data);
        setDevices(devRes.data.data);
      } else if (activeTab === "subscription") {
        const res = await apiClient.get(`/admin/users/${id}/subscription`);
        setSubscription(res.data.data);
      } else if (activeTab === "notes") {
        const res = await apiClient.get(`/admin/users/${id}/notes`);
        setNotes(res.data.data);
      } else if (activeTab === "activity") {
        const [actRes, auditRes] = await Promise.all([
          apiClient.get(`/admin/users/${id}/activity`),
          apiClient.get(`/admin/users/${id}/audit`),
        ]);
        setActivities(actRes.data.data);
        setAuditLogs(auditRes.data.data);
      } else if (activeTab === "timeline") {
        const res = await apiClient.get(`/admin/users/${id}/timeline`);
  ...
  useEffect(() => {
    if (id && activeTab !== "overview") {
      apiClient.get(`/admin/users/${id}/overview`)
  ...
  const handleToggleBlock = () => {
    ...
    setConfirmAction({
      ...
      onConfirm: async () => {
        try {
          await apiClient.patch(`/admin/users/${id}/block`, { isBlocked: nextBlocked });
          showToast(`Account for ${overview.name} is now ${nextBlocked ? "blocked" : "active"}`, "success");
          fetchSubResource();
        } catch (err: any) {
  ...
  const handleForcePasswordReset = () => {
    setConfirmAction({
      ...
      onConfirm: async () => {
        try {
          await apiClient.post(`/admin/users/${id}/force-password-reset`);
  ...
  const handleRevokeAllSessions = () => {
    setConfirmAction({
      ...
      onConfirm: async () => {
        try {
          await apiClient.delete(`/admin/users/${id}/sessions`);
  ...
  const handleAddNote = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post(`/admin/users/${id}/notes`, { note: noteText });
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-4">
                    {[
                      ...
                      { label: "Mobile Number", value: overview.phone || "N/A" },
                      { label: "Company Name", value: overview.companyName || "N/A" },
                    ].map((item, idx) => (
                  ...
                  <div className="space-y-4">
                    {[
                      { label: "Verification Status", value: overview.emailVerified ? "Verified ✅" : "Pending Verification ⚠️" },
                      { label: "Country Origin", value: overview.country || "N/A" },
                      { label: "Registered At", value: new Date(overview.createdAt).toLocaleString() },
                      { label: "Last Login Active", value: overview.lastLoginAt ? new Date(overview.lastLoginAt).toLocaleString() : "Never" },
                    ].map((item, idx) => (
                      <div key={idx} className="border-b border-border/60 pb-3">
                        <span className="text-xs font-bold text-text-light block uppercase tracking-wider">{item.label}</span>
                        <span className="text-sm font-semibold text-text mt-1 block">{item.value}</span>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-border pb-4">
                  <div>
                    ...
                    <p className="text-xs text-text-light">Password compliance, MFA status, and admin role assignments.</p>
                  </div>
                  <button
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-4">
                    ...
                    {[
                      { label: "Last Password Changed At", value: security?.passwordChangedAt ? new Date(security.passwordChangedAt).toLocaleString() : "Never" },
                      { label: "Forced Password Reset Active", value: security?.forcePasswordChange ? "Yes (Requires reset on next login)" : "No" },
                      { label: "Multi-Factor Authentication (MFA)", value: security?.twoFactorEnabled ? "Enabled" : "Disabled" },
                    ].map((item, idx) => (
                  ...
                  <div className="space-y-4">
                    ...
                    {roles.length === 0 ? (
                      <p className="text-xs text-text-light italic">No admin roles assigned to this account.</p>
                    ) : (
                      <div className="space-y-3">
                        {roles.map(r => (
                          <div key={r.id} className="p-3 border border-border rounded-xl bg-background/50 flex items-center justify-between">
                            <div>
                              <span className="font-bold text-sm text-text block">{r.role.name}</span>
                              <span className="text-xs text-text-light block">{r.role.description}</span>
                              {r.expiresAt && (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-border pb-4">
                  <div>
                    ...
                    <p className="text-xs text-text-light">Manage and revoke active authorization sessions globally.</p>
                  </div>
                  <button
                        ...
                        <div key={s.id} className="p-4 border border-border rounded-xl bg-background/50 flex flex-col justify-between space-y-3">
                          <div>
                            <div className="flex items-center justify-between">
                              <span className="font-bold text-sm">{s.operatingSystem} • {s.browser}</span>
                              {s.isActive && (
                            ...
                            <span className="text-xs text-text-light block mt-1">IP: {s.ipAddress}</span>
                            <span className="text-xs text-text-light block">Location: {s.location || "Unknown"}</span>
                          </div>
                ...
                <div className="space-y-4 pt-4 border-t border-border">
                  ...
                  {devices.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {devices.map(d => (
                        <div key={d.id} className="p-3 border border-border/60 rounded-xl flex items-center justify-between text-xs">
                          <div className="flex items-center gap-3">
                            <Smartphone size={20} className="text-text-light" />
                            <div>
                              <span className="font-bold text-sm text-text block">{d.deviceName}</span>
                              <span className="text-text-light block">OS: {d.osName} • Browser: {d.browserName}</span>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-text-light block">Last IP: {d.lastIpAddress}</span>
                            <span className="text-[10px] text-text-light block">Last Active: {new Date(d.lastActiveAt).toLocaleString()}</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {activeTab === "subscription" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Current subscription level, period range, and statuses.</p>
                </div>
                ...
                <div className="p-5 border border-border rounded-2xl bg-background/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-light">Active Subscription Plan</span>
                  ...
                  <div className="text-xs font-semibold text-text-light space-y-1 sm:text-right">
                    <div>Status: <span className="text-emerald-500 font-bold uppercase">{subscription?.status || "ACTIVE"}</span></div>
                    {subscription?.currentPeriodEnd && (
                      <div>Period Ends: {new Date(subscription.currentPeriodEnd).toLocaleDateString()}</div>
                    )}
                    {subscription?.cancelAtPeriodEnd && (
                      <div className="text-rose-500 font-bold">Auto-Renew: Cancelled at Period End</div>
                    )}
                  </div>
                </div>
              </div>
            ...
            {activeTab === "notes" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Private team logs and comments regarding this user account.</p>
                </div>
                ...
                <div className="space-y-3 pt-4 border-t border-border">
                  {notes.length === 0 ? (
                    <p className="text-xs text-text-light italic text-center py-4">No internal notes logged for this user.</p>
                  ) : (
                    notes.map(n => (
                      <div key={n.id} className="p-4 border border-border/80 rounded-xl bg-background/50 space-y-2">
                        <div className="flex items-center justify-between text-xs">
                          <span className="font-bold text-text">{n.author.name} ({n.author.email})</span>
                          <span className="text-text-light">{new Date(n.createdAt).toLocaleString()}</span>
                        </div>
            ...
            {activeTab === "activity" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Track login activity, changes, and audit operations relating to this user.</p>
                </div>

                <div className="grid gap-6 lg:grid-cols-2">
                  {/* Activity log */}
                  <div className="space-y-4">
                    <h4 className="text-sm font-bold text-text border-b border-border pb-2">User Activities</h4>
                    {activities.length === 0 ? (
                      <p className="text-xs text-text-light italic">No activity logged.</p>
                    ) : (
                      <div className="space-y-3">
                        {activities.map(a => (
                          <div key={a.id} className="p-3 border border-border/60 rounded-xl bg-background/30 text-xs">
                            ...
                            <span className="text-[10px] text-text-light block mt-1">IP: {a.ipAddress}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  {/* Audit log */}
                  <div className="space-y-4">
                    <h4 className="text-sm font-bold text-text border-b border-border pb-2">Admin Audit Actions</h4>
                    {auditLogs.length === 0 ? (
                      <p className="text-xs text-text-light italic">No admin audit events.</p>
                    ) : (
                      <div className="space-y-3">
                        {auditLogs.map(a => (
                          <div key={a.id} className="p-3 border border-border/60 rounded-xl bg-background/30 text-xs space-y-1">
                            <div className="flex justify-between items-center font-bold">
                              <span className="text-text">{a.action}</span>
                              <span className="text-text-light font-normal text-[10px]">{new Date(a.createdAt).toLocaleString()}</span>
                            </div>
            ...
            {activeTab === "timeline" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-text-light">Chronological feed of key lifecycle events since registration.</p>
                </div>

                <div className="relative pl-6 border-l-2 border-primary/20 space-y-6">
                  {timeline.length === 0 ? (
                    <p className="text-xs text-text-light italic pl-2">No timeline events recorded.</p>
                  ) : (
                    timeline.map(t => (
