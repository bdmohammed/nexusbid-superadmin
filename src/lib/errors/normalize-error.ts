...
export function normalizeError(error: unknown): AppError {
  ...
  if (error instanceof Error) {
    ...
    if (error.name === 'ZodError' || 'issues' in error) {
      ...
      return new ValidationError('Validation failed', { fieldErrors, issues }, { cause: error });
    }
  ...
  if (isAxiosError(error)) {
    if (error.response) {
      ...
      switch (status) {
        ...
        default:
          return new ApiError(apiMessage, status, { responseData }, { cause: error });
      }
    } else if (error.request) {
      // Request was made but no response received
      return new NetworkError('No response received from API server', { request: error.request }, { cause: error });
    }
  }
  ...
  return new UnknownError('An unexpected server error occurred', { rawError: error });
}
