    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: (params: ICellRendererParams<Role>) => {
          const r = params.data;
          if (!r) return null;
          const replacesMatch = r.description?.match(
            /\[ReplacesRole:\s*([0-9a-fA-F-]+)\]/,
          );
          const cleanDesc =
            r.description
        ...
        cellRenderer: (params: ICellRendererParams<Role>) => {
          ...
          return (
            <div className="flex items-center h-full text-xs font-semibold text-text">
              {label}
              {tag && (
                <span className="text-text-light font-normal text-[11px] ml-1">
                  {tag}
                </span>
              )}
            </div>
      ...
      },
      {
        ...
        cellRenderer: (params: ICellRendererParams<Role>) => {
          ...
          if (!user) {
            return (
              <span className="text-xs text-text-light italic">System</span>
            );
          }
          return (
      ...
      },
      {
        ...
        cellRenderer: (params: ICellRendererParams<Role>) => {
          ...
          return (
            <div className="flex items-center h-full text-xs text-text-light">
              {r.updatedAt
                ? dayjs(r.updatedAt).format("DD MMM YYYY, hh:mm A")
                : "—"}
            </div>
          );
        },
