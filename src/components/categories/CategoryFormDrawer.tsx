...
interface CategoryFormDrawerProps {
  ...
  setCurrentCategoryForm: React.Dispatch<
    React.SetStateAction<Partial<Category>>
  >;
  categories: Category[];
...
export const CategoryFormDrawer: React.FC<CategoryFormDrawerProps> = ({
  ...
  const handlePostComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await categoryApi.addCategoryComment(
        selectedCategory.id,
        newCommentText.trim(),
      );
      if (res.data?.success) {
      ...
      toast.error(
        err?.response?.data?.message || err?.message || "Failed to post note",
      );
    } finally {
  ...
  const assignments =
    ...
    (selectedCategory as any)?.reviewAssignments ||
    [];
  ...
  const isAssignedReviewer =
    ...
    (Boolean(activeUserId) && reviewerIds.includes(activeUserId!));

  const categoryStatus =
    selectedCategory?.status || currentCategoryForm.status || "DRAFT";
      ...
      <div className="fixed right-0 top-0 z-[100] flex h-screen w-full max-w-2xl flex-col bg-surface shadow-2xl transition-all duration-300 border-l border-border overflow-hidden text-text">
          ...
          {[
            ...
            return (
              <button
                ...
                onClick={() =>
                  tab.isUnlocked && setDrawerTab(tab.id as DrawerTab)
                }
                title={
                  !tab.isUnlocked
                    ? "Save draft category first to unlock workflow governance"
                    : ""
                }
            ...
            <div className="space-y-4">
              {isInReview && (
                <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-300 text-xs flex items-center gap-2 font-medium">
                  ...
                  <span>
                    <strong>Status: In Review</strong> — Editing and saving
                    draft changes are disabled while this category is under
                    governance review.
                  </span>
                </div>
              )}
              ...
              <Input
                ...
                placeholder={
                  drawerMode === "create"
                    ? "Auto-generated on Save (e.g. 001)"
                    : "001"
                }
            ...
            <div className="space-y-4">
              {isInReview && (
                <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-300 text-xs flex items-center gap-2 font-medium">
                  <Lock className="h-4 w-4 shrink-0 text-amber-500" />
                  <span>
                    <strong>Status: In Review</strong> — Hierarchy configuration
                    is locked during governance review.
                  </span>
                </div>
              )}
                      ...
                      ? {
                          ...
                          label: (() => {
                            const p = categories.find(
                              (c) => c.id === currentCategoryForm.parentId,
                            );
                            return p
                              ? `${p.code} - ${p.name}`
                              : currentCategoryForm.parentId;
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-text-light mt-0.5">
                      Submit draft version to designated reviewers for
                  ...
                  {selectedCategory?.status === "PUBLISHED" ? (
                    <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 text-xs flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-text-light mt-0.5">
                          This category has passed Maker-Checker review and is
                    ...
                    <div className="space-y-2">
                      ...
                      {!selectedCategory?.id && (
                        <p className="text-[11px] text-amber-600 dark:text-amber-400 font-medium flex items-center gap-1">
                          ⚠️ Please click "Save Draft Changes" in the footer
                          first before submitting for governance review.
                        </p>
                      )}
                    </div>
                  ) : (
                    <div className="p-3.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 text-xs flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-text-light mt-0.5">
                          Assigned reviewers must approve or request changes for
                          this category version.
                        </div>
                        {!isAssignedReviewer && (
                          <p className="text-[11px] text-rose-600 dark:text-rose-400 font-medium flex items-center gap-1 mt-1">
                            ⚠️ Only designated assigned reviewers can execute a
                            review decision on this draft.
                          </p>
                        )}
                      </div>
                      <Button
                ...
                {reviews.flatMap((r) => r.comments || []).length === 0 ? (
                  <div className="p-8 text-center border border-dashed border-border rounded-2xl bg-surface/50">
                    ...
                    <p className="text-xs font-semibold text-text">
                      No Governance Comments
                    </p>
                    <p className="text-[11px] text-text-light mt-0.5">
                      Comments and notes from reviewers will appear here during
                      governance review.
                    </p>
                  </div>
                ) : (
                  reviews
                    ...
                    .map((c) => (
                      ...
                      >
                        ...
                        <p className="text-text-light pl-8 italic">
                          "{c.comment}"
                        </p>
                      </div>
                    ))
          ...
          {drawerTab === "versions" && (
            <div className="flex-1 overflow-y-auto space-y-6 max-w-3xl">
              <div className="flex items-center justify-between border-b border-border pb-4">
                <div>
                  <h3 className="text-base font-bold text-text flex items-center gap-2">
                    <History className="h-5 w-5 text-primary" /> Category
                    Version Releases
                  </h3>
                  <p className="text-xs text-text-light">
          ...
          {drawerTab === "activity" && (
            <div className="flex-1 overflow-y-auto space-y-6 max-w-3xl">
              <div className="flex items-center justify-between border-b border-border pb-4">
                <div>
                  <h3 className="text-base font-bold text-text flex items-center gap-2">
                    <Clock className="h-5 w-5 text-primary" /> Category Activity
                    Log & Audit Trail
                  </h3>
                  <p className="text-xs text-text-light">
                    Forensic timeline of category creations, hierarchy
          ...
          {drawerTab === "usage" && (
            <div className="flex-1 overflow-y-auto space-y-6 max-w-3xl">
              <div className="flex items-center justify-between border-b border-border pb-4">
                <div>
                  <h3 className="text-base font-bold text-text flex items-center gap-2">
                    <BarChart2 className="h-5 w-5 text-primary" /> Category
                    Impact & Analytics
                  </h3>
                  <p className="text-xs text-text-light">
                    Real-time metrics for tenders and child subcategories bound
                    to this category.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="p-5 rounded-2xl border border-border bg-surface text-center space-y-1 shadow-xs">
                  ...
                  <h4 className="text-3xl font-bold text-text">
                    {usageStats.tendersCount}
                  </h4>
                </div>
        ...
        <div className="border-t border-border bg-surface px-6 py-4 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {(drawerTab === "general" || drawerTab === "hierarchy") &&
              drawerMode !== "view" && (
                <Button
                  type="button"
                  size="sm"
