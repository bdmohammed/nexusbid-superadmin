...
export const countriesApi = {
  ...
  getTimeline(countryId: string, stateId?: string) {
    return apiClient.get<ApiResponse<ActivityItem[]>>(`/countries/${countryId}/timeline`, {
      params: stateId ? { stateId } : undefined,
    });
  },

  getCountriesHierarchy() {
    return apiClient.get<ApiResponse<CountryHierarchyNode[]>>('/countries/hierarchy');
  },
  ...
  getEligibleReviewers() {
    return apiClient.get<ApiResponse<Reviewer[]>>('/countries/eligible-reviewers');
  },

  getDependencyMatrix(query: DependencyMatrixQueryInput) {
    return apiClient.get<ApiResponse<DependencyMatrix>>('/countries/dependency-matrix', {
      params: query,
    });
  },

  getReviewsQueue(query?: ChangeRequestQueryInput) {
    return apiClient.get<ApiResponse<TicketItem[]>>('/countries/change-requests', {
      params: query,
    });
  },

  getChangeRequestDetails(requestId: string) {
    return apiClient.get<ApiResponse<TicketItem>>(`/countries/change-requests/${requestId}`);
  },

  createChangeRequest(input: CreateCountryChangeRequestInput) {
    return apiClient.post<ApiResponse<TicketItem>>('/countries/change-requests', input);
  },
  ...
  updateState(stateId: string, input: UpdateStateBodyDto) {
    return apiClient.patch<ApiResponse<State>>(`/countries/states/${stateId}`, input);
  },
